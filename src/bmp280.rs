...
impl BMP280 {
    ...
    pub fn new_with_address<E, I2C>(i2c: &mut I2C, addr: u8) -> Result<BMP280, E>
    where
    ...
    pub fn new<E, I2C>(i2c: &mut I2C) -> Result<BMP280, E>
    where
        I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
...
impl BMP280 {
    fn read_calibration<I2C, E>(&mut self, i2c: &mut I2C)
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let mut data: [u8; 24] = [0; 24];
        let _ = i2c.write_read(self.addr, &[Register::calib00 as u8], &mut data);
    ...
    pub fn pressure<I2C, E>(&mut self, i2c: &mut I2C) -> f64
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let mut data: [u8; 6] = [0, 0, 0, 0, 0, 0];
        let _ = i2c.write_read(self.addr, &[Register::press as u8], &mut data);
        ...
        // let mut p = 1_048_576 - (press as i64);
    ...
    pub fn temp<I2C, E>(&mut self, i2c: &mut I2C) -> f64
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let mut data: [u8; 6] = [0, 0, 0, 0, 0, 0];
    ...
    pub fn config<I2C, E>(&mut self, i2c: &mut I2C) -> Config
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let config = self.read_byte(i2c, Register::config);
    ...
    /// Sets configuration
    pub fn set_config<I2C, E>(&mut self, i2c: &mut I2C, new: Config)
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let config: u8 = 0x00;
        ...
        self.write_byte(i2c, Register::config, config | t_sb | filter);
    }

    /// Sets control
    pub fn set_control<I2C, E>(&mut self, i2c: &mut I2C, new: Control)
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        ...
        self.write_byte(i2c, Register::ctrl_meas, control);
    }

    /// Returns control
    pub fn control<I2C, E>(&mut self, i2c: &mut I2C) -> Control
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let config = self.read_byte(i2c, Register::ctrl_meas);
    ...
    pub fn status<I2C, E>(&mut self, i2c: &mut I2C) -> Status
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>{
        let status = self.read_byte(i2c, Register::status);
        Status {
    ...
    /// Returns device id
    pub fn id<I2C, E>(&mut self, i2c: &mut I2C) -> u8
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>{
        self.read_byte(i2c, Register::id)
    }

    /// Software reset, emulates POR
    pub fn reset<I2C, E>(&mut self, i2c: &mut I2C)
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        self.write_byte(i2c, Register::reset, 0xB6); // Magic from documentation
    }

    fn write_byte<I2C, E>(&mut self, i2c: &mut I2C, reg: Register, byte: u8)
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let mut buffer = [0];
        let _ = i2c.write_read(self.addr, &[reg as u8, byte], &mut buffer);
    }

    fn read_byte<I2C, E>(&mut self, i2c: &mut I2C ,reg: Register) -> u8
    where
    I2C: ehal::blocking::i2c::WriteRead<Error = E>
    {
        let mut data: [u8; 1] = [0];
        let _ = i2c.write_read(self.addr, &[reg as u8], &mut data);
        data[0]
    }
}
